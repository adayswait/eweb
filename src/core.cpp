...
WebBrowser::WebBrowser(HWND parentWindowHandle_)
{
    ...
    {
        this->Navigate(const_cast<wchar_t *>(HOME_PAGE));
    }
}
...
bool WebBrowser::CreateBrowser()
{
    ...
    webBrowser2->put_Silent(VARIANT_TRUE); //屏蔽js错误提示
...
void WebBrowser::TranslateMessage(LPMSG lpmsg)
{
    if (olePlaceActiveObject != nullptr)
    {
        olePlaceActiveObject->TranslateAcceleratorA(lpmsg);
    }
}
                                       ...
                                       ApiToJavaScript cppFunctionPtr)
{
    ...
    jsFunc.jsFunctionID = DISPID(DISPID_BASE + javaScriptFunctions.size());
                                                    ...
                                                    DISPID *rgDispId)
{
    ...
    size_t functionCount = javaScriptFunctions.size();
...
HRESULT STDMETHODCALLTYPE WebBrowser::Invoke(DISPID dispIdMember,
                                             ...
                                             WORD wFlags, DISPPARAMS *Params,
                                             VARIANT *pVarResult,
                                             EXCEPINFO *pExcepInfo, UINT *puArgErr)
{
    if (dispIdMember < DISPID_BASE ||
        dispIdMember > DISPID(DISPID_BASE + javaScriptFunctions.size()))
    {
        return E_NOTIMPL;
    }
    ...
    return javaScriptFunctions[dispIdMember - DISPID_BASE]
        .cppFunctionPtr(dispIdMember, Params, pVarResult);
}
